classdef CalculateReferencePose < matlab.System
    ...
    methods(Access = protected)
        ...
        function [CurvatureSequence, LateralDeviation, RelativeYawAngle, IsGoalReached] = stepImpl(obj, EgoActor, EgoReferencePath)
            ...
            obj.NumWaypoints = EgoReferencePath.numPoints;
            ...
            if endIndex < (obj.NumWaypoints-2)
                IsGoalReached = false;
            else
                IsGoalReached = true;
                return;
            ...
            ArcLength0 = obj.ReferencePath.arcLength(startIndex);
            ...
            ArcLength1 = obj.ReferencePath.arcLength(endIndex);
            ...
            % velocity
            ArcLengthCurr = ArcLength0 + u*(ArcLength1-ArcLength0);
            [CurvatureSequence] = getCurvatureSeq(obj,ArcLengthCurr,EgoActor.Velocity);
        ...
        function [CurvatureSequence] = getCurvatureSeq(obj,ArcLengthCurr,Velocity)
            % calculate the curvature sequence based on the current arc
            % length and current ego velocity.
            speed = norm(Velocity(1:2));
            ArcLengthPrediction = zeros(obj.PredictionHorizon,1);

            ArcLengthPrediction(1) = ArcLengthCurr;
            for i = 2:obj.PredictionHorizon
                ArcLengthPrediction(i) = ArcLengthCurr + obj.SampleTime*(i-1)*speed;
            end
            CurvatureSequence = interp1(obj.ReferencePath.arcLength(1:obj.NumWaypoints), obj.ReferencePath.kappa(1:obj.NumWaypoints), ArcLengthPrediction,'linear','extrap');
        end
