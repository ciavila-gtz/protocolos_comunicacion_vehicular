...
%   planNextSegment     - Determine if ready to plan for next path segment 
...
classdef HelperBehavioralPlanner < handle
    ...
    methods
        ...
        %------------------------------------------------------------------
        function tf = planNextSegment(this, currentPose, distance)
            %planNextSegment

            if ~reachedDestination(this) && norm(currentPose(1:2) - this.RoutePlan(this.GoalIndex).StartPose(1:2)) < distance
                tf = true;
            else
                tf = false;
            end
