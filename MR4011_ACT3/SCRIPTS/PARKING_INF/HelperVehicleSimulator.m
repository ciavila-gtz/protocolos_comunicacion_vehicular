...
%   getVehicleAngilarVelocity   - Get vehicle angular velocity (in rad/second)
...
%   showLegend              - Show or hide legend in plot
...
classdef HelperVehicleSimulator < handle
    ...
    properties (Access = private)
        ...
        %TrajectoryHandle Graphics handle for reference path
        RefPathHandle = []

        %TrajectoryHandle Graphics handle for local path
        LocalPathHandle = []
    end
    ...
    methods
        ...
        %------------------------------------------------------------------
        function showLegend(obj, showLegend)

            validateattributes(showLegend, {'numeric','logical'},...
                {'scalar','nonempty','binary'}, 'showLegend',...
                'showTrajectory');
            
            if isgraphics(obj.Axes)
                if showLegend
                    legend(obj.Axes,'show','Location','northwest');
                else
                    legend(obj.Axes,'hide');
                end
            end
        end

        %------------------------------------------------------------------
        function plotReferencePath(obj, refPath)
            validateattributes(refPath, {'numeric'},...
                {'2d','ncols',3}, 'plotReferencePath',...
                'refPath');

            updateRefPathPlot(obj, refPath);
        end

        %------------------------------------------------------------------
        function clearReferencePath(obj)
            if isgraphics(obj.RefPathHandle)
                delete(obj.RefPathHandle);
            end
        end

        %------------------------------------------------------------------
        function plotLocalPath(obj, localPath)
            validateattributes(localPath, {'numeric'},...
                {'2d','ncols',3}, 'plotLocalPath',...
                'localPath');

            updateLocalPathPlot(obj, localPath);
        end

        %------------------------------------------------------------------
        function clearLocalPath(obj)
            if isgraphics(obj.LocalPathHandle)
                delete(obj.LocalPathHandle);
            end
        end
        ...
        %------------------------------------------------------------------
        function omega = getVehicleAngularVelocity(obj)
            %getVehicleVelocity Return longitudinal velocity of the vehicle
            ...
            omega = obj.Vehicle.AngularVelocity;
        end
    ...
    methods (Access = private)
        ...
        function plotTrajectory(obj)
            ...
            if isempty(obj.TrajectoryHandle) || ~isgraphics(obj.TrajectoryHandle)
                obj.TrajectoryHandle = plot(obj.Axes, trajectory(:,1), trajectory(:,2),'Color','r','LineWidth',3,'DisplayName','Vehicle Trajectory');
            else
        ...
        %------------------------------------------------------------------
        function updateRefPathPlot(obj, refPath)
            %plotTrajectory Plot vehicle reference path

            if ~isgraphics(obj.Axes)
                return
            end
            
            if isempty(obj.RefPathHandle) || ~isgraphics(obj.RefPathHandle)
                obj.RefPathHandle = plot(obj.Axes, refPath(:,1), refPath(:,2),'Color','b','LineWidth',2,'DisplayName','Reference Path');
            else
                obj.RefPathHandle.XData = refPath(:,1);
                obj.RefPathHandle.YData = refPath(:,2);
            end
        ...
        %------------------------------------------------------------------
        function updateLocalPathPlot(obj, localPath)
            %plotTrajectory Plot vehicle local path

            if ~isgraphics(obj.Axes)
                return
            end
            
            if isempty(obj.LocalPathHandle) || ~isgraphics(obj.RefPathHandle)
                obj.LocalPathHandle = plot(obj.Axes, localPath(:,1), localPath(:,2),'Color','g','LineWidth',2,'DisplayName','Local Path');
            else
                obj.LocalPathHandle.XData = localPath(:,1);
                obj.LocalPathHandle.YData = localPath(:,2);
            end
            
            %uistack(obj.LocalPathHandle, 'top');
